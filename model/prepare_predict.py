...
def prepare_predict(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[['Уставной капитал (руб)', 'Адрес',
             ...
             'Планируемый оборот по снятию д/с (руб)', 'Доходы (тыс, руб.)',
    ...
    df['Основной ОКВЭД'] = df['Основной ОКВЭД'].astype(str).str.split('.').str[0].astype('category')
    ...
    df['Негатив относительно ГД'] = df['Негатив относительно ГД'].astype(int)
    df['Негативная информация'] = df['Негативная информация'].astype(int)
    ...
    df = df[['Уставной капитал (руб)', 'Адрес', 'ФИО Бенефициара',
             'Основной ОКВЭД', 'Кол-во дополнительных ОКВЭДОВ', 'Кол-во сотрудников',
             'Система налогообложения ', 'Мошенники',
             'Сервисы регистраторы',
             'Срок жизни SIM-карты/номера (количество дней/часов/минут, которое прошло от даты заключения договора)',
